...
var arraySum = function(array) {
  ...
  if (Array.isArray(current)) {
    return arraySum(arr) + arraySum(current);
  } else {
    return current + arraySum(arr);
  }
};
...
var range = function(x, y) {
  ...
  results.push(num);
  return results.concat(range(num, y));
};
...
var reverse = function(string) {
  if (string.length === 1) {
    ...
    var char = string.substr(-1);
    var newString = string.substr(0, string.length - 1);
    return char + reverse(newString);
  }
};
...
var palindrome = function(string) {
  ...
  if (string.length === 1 || string.length === 0) {
    ...
    var newString = string.substr(1, string.length - 2);
...
var modulo = function(x, y) {
  if (x < 0 && y < 0) {
    var positiveX = x - x - x;
    var positiveY = y - y - y;
    var positiveRemainder = modulo(positiveX, positiveY);
    return positiveRemainder - positiveRemainder - positiveRemainder;
  } else if (x < 0) {
    var positiveX = x - x - x;
    var positiveRemainder = modulo(positiveX, y);
    return positiveRemainder - positiveRemainder - positiveRemainder;
  } else if (y < 0) {
    var positiveY = y - y - y;
    var positiveRemainder = modulo(x, positiveY);
    return positiveRemainder - positiveRemainder - positiveRemainder;
  } else if (x === 0 && y === 0) {
    return NaN;
  } else if (x === 0) {
...
var createArray = function(str) {
  if (!str.length) {
    ...
    return [str[0]].concat(createArray(newStr));
  }
};
